document.addEventListener('DOMContentLoaded', function () {
                ...
                http.onreadystatechange = function () {
                    if (http.readyState === XMLHttpRequest.DONE) {
                        if (http.status === 204 || http.status === 200) {
                            window.location.replace("/rest/settings");
                        } else {
    ...
    if (profileManagementPage.length > 0) {
        document.querySelector("#link-strava").setAttribute("href", "https://www.strava.com/oauth/authorize?client_id=35158&redirect_uri="
            + window.location.protocol + "//" + window.location.host + "/rest/settings/link_strava&response_type=code&scope=read,read_all,profile:read_all,profile:write,activity:read_all,activity:write");
    ...
    function submitLoginForm() {
        ...
        if (username !== "" && password !== "") {
            ...
            http.onreadystatechange = function () {
                if (http.readyState === XMLHttpRequest.DONE) {
                    ...
                    if (http.status === 200) {
                        ...
                        window.location.replace("/rest/security/profile/" + usernameFromToken);
            ...
            http.open("POST", "/rest/login", true);
            http.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        ...
        function submitResetEnterForm() {
            ...
            if (recoveryToken !== "" && recoveryPassword !== "" && recoveryPassword === recoveryPasswordConfirm) {
                ...
                http.onreadystatechange = function () {
                    if (http.readyState === XMLHttpRequest.DONE) {
                        if (http.status === 200) {
                            window.location.replace("/rest/login?message=reset_success");
                        } else if (http.status === 401) {
                            window.location.replace("/rest/login?error=not_authorized");
                        } else if (http.status === 402) {
                            window.location.replace("/rest/login?error=not_activated");
                        } else if (http.status === 404) {
                            window.location.replace("/rest/login?error=reset_token_invalid");
                        } else {
                            console.log("Response: " + http.status);
                        }
                    }
                ...
                http.open("POST", "/rest/password/reset/enter", true);
                http.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                http.setRequestHeader('Accept', 'text/html');
                http.setRequestHeader('Cache-Control', 'no-store');
                http.send(params);
        ...
        function handleResetRequestFormSubmit() {
            ...
            if (username !== "") {
                ...
                http.onreadystatechange = function () {
                    if (http.readyState === XMLHttpRequest.DONE) {
                        if (http.status === 200) {
                            window.location.replace("/rest/login?message=reset_request_success");
                        } else if (http.status === 401) {
                ...
                http.open("POST", "/rest/password/reset", true);
                http.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                http.setRequestHeader('Accept', 'text/html');
                http.setRequestHeader('Cache-Control', 'no-store');
                http.send(param);
    ...
    if (tokenJson !== null && tokenJson.exp > Math.floor(Date.now() / 1000)) {
        indexNavUl.append("<li class='nav-item'><a href='/rest/security/profile/" + usernameFromToken + "'>profile</a></li>");
        indexNavUl.append("<li class='nav-item'><a href='/rest/logout'>sign out</a></li>");
    } else {
        indexNavUl.append("<li class='nav-item'><a href='/rest/register'>sign up</a></li>");
    ...
    function uploadImage(id) {
        ...
        $.ajax({
            url: "/rest/profiles/" + usernameFromToken + "/picture",
            type: "POST",
            data: image,
            processData: false,
            contentType: false,
    ...
    function getImage(username) {
        var http = new XMLHttpRequest();
        var url = "/rest/profiles/" + username + "/picture";
    ...
    if ($("#register").length > 0) {
        ...
        validate.validators.usernameAvailabilityValidator = function (value) {
            return new validate.Promise(function (resolve, reject) {
                ...
                http.open("GET", "/rest/register/username?" + param, true);
                http.setRequestHeader('Accept', 'application/json');
                http.setRequestHeader('Cache-Control', 'no-store');
                http.send();
            });
        ...
        validate.validators.emailAvailabilityValidator = function (value) {
            return new validate.Promise(function (resolve, reject) {
                ...
                http.open("GET", "/rest/register/email?" + param, true);
                http.setRequestHeader('Accept', 'application/json');
                http.setRequestHeader('Cache-Control', 'no-store');
                http.send();
            });
            ...
            http.onreadystatechange = function () {
                if (http.readyState === XMLHttpRequest.DONE) {
                    if (http.status === 200) {
                        ...
                        window.location.replace("/rest/login?message=registration_success");
    ...
    function loadFavoriteLayoutOptions() {
        ...
        http.open("GET", "/rest/profiles/" + usernameFromToken + "/favorite/name", true);
        http.setRequestHeader('Cache-Control', 'no-store');
        http.send();
    }
    ...
    function upgradeToPremium() {
        ...
        http.onreadystatechange = function () {
            if (http.readyState === XMLHttpRequest.DONE) {
                if (http.status === 200) {
                    window.location.replace("/rest/profiles");
                } else {
                    console.log("Response: " + http.status);
                }
            }
        ...
        http.open("POST", "/rest/premium/join", true);
        http.setRequestHeader('Accept', 'text/html');
        http.setRequestHeader('Cache-Control', 'no-store');
        http.send();
    }
